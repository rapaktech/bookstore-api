        ...
        if (userName != "admin") {
            ...
            if (foundUser) {
                ...
                if (done === true) {
                    ...
                    callback(400, { message: "Write error occured. Please try again later" });
                }
            } else {
                callback(400, { message: "User not found" });
            }
        } else {
            callback(400, { message: "Username can't be admin" });
        }
    } else {
        callback(400, { message: "Please check your info and try again" });
    }
};
        ...
        if (user) {
            ...
            if (book) {
                if (book.copies <= 0) {
                    ...
                    if (user.books.find(book => book.name == bookName && book.author == bookAuthor)) {
                        callback(400, { message: "You've already borrowed this book before."});
                    } else {
                        book.copies--;
                        ...
                        if (doneUsers === true && doneBooks === true) {
                            book.id = null;
                            book.copies = null;
                            callback(200, { message: 'book borrowed successfully', data: book });
                        } else {
                            callback(400, { message: 'Some error occured. Please try again later'});
                        }
                    }
...
routeHandler._books.put = (data, callback) => {
    ...
    if (data.query.return == 'yes') {
        ...
        if (user) {
            let book = Books.find(book => book.name == bookName && book.author == bookAuthor);
            if (book) {
                let userBorrowedBook = user.books.find(book => book.name == bookName && book.author == bookAuthor);
                if (userBorrowedBook) {
                    ...
                    if (doneUsers === true && doneBooks === true) {
                        ...
                        callback(400, { message: "You haven't borrowed this book yet"});
                    }
            } else {
